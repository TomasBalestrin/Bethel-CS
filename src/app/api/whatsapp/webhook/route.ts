  ...
  try {
    ...
    if (event === 'message_received' || event === 'messages.upsert' || event === 'message' || event === 'new_message' || event === 'message_sent' || event === 'outgoing_message') {
      ...
      if (insertErr) {
        ...
        // Persiste em system_errors pra admin ver sem depender do log Vercel.
        await logOpError({
          ...
          context: { direction, messageType, contentLen: content?.length ?? 0, hasMediaUrl: !!mediaUrl, channel, raw: data },
        })
